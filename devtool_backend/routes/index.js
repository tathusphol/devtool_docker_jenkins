...
router.post("/volunteer_activity", async function (req, res, next) {
  try {
    const [data, field] = await pool.query("SELECT * FROM user_activity AS ua LEFT OUTER JOIN activity AS a ON a.a_id = ua.a_id WHERE ua.u_id = ? AND a.status = ?", [
      req.body.u_id, "stop"
    ]);
    console.log(data)
    return res.json(data);
...
router.post("/volunteer_activity_ongoing", async function (req, res, next) {
  try {
    const [data, field] = await pool.query("SELECT * FROM user_activity AS ua LEFT OUTER JOIN activity AS a ON a.a_id = ua.a_id WHERE ua.u_id = ? AND a.status = ?", [
      req.body.u_id, "open"
    ]);
...
router.post("/charity_completed", async function (req, res, next) {
  try {
    const [data, field] = await pool.query("SELECT * FROM activity WHERE u_id = ? AND status = ?", [
      req.body.u_id, "stop"
    ]);
